  ...
  <HomePanel title="最新专题">
    ...
    <div ref="target" class="special-list">
      <div class="special-item" v-for="item in specialList" :key="item.id">
        ...
        <div class="foot">
          <span class="like"><i class="iconfont icon-response-fill"></i>{{item.collectNum}}</span>
          <span class="view"><i class="iconfont icon-editor-left-alignment"></i>{{item.viewNum}}</span>
          <span class="reply"><i class="iconfont icon-editor-left-alignment"></i>{{item.replyNum}}</span>
        </div>
      </div>
    </div>
  </HomePanel>

...
export default {
  ...
  mutations: {
    ...
    insertCart (state, payload) {
      ...
      // 他们分别是：id skuId name attrsText picture price nowPrice selected stock count count
      // 插入数据规则
      // 1. 先找下是否有相同商品
      // 2. 如果有相同的商品，查询他的数量，累加到payload上，在保存到最新位置，原来 商品商品需要删除
    ...
    findCart (ctx) {
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) {
          ...
          // dataList 成功结果的集合，数据顺序和promiseArr顺序一致，她又是根据state.list而来
